...
class LogisticPolicyNetwork(ActionDecoder):
    def __init__(
        ...
        print("n_dist", self.n_dist)
        self.prob_fc = nn.Linear(hidden_size, out_features * self.n_dist) # hidden size 2048, out_features 7, n_dist 10
    ...
    def forward(  # type: ignore
        ...
        # latent_plan = torch.ones(latent_plan.shape).to(perceptual_emb.device) # Added to ommit latent plan
        # latent_goal = torch.ones(latent_goal.shape).to(perceptual_emb.device) # Added to ommit latent goal
        x = torch.cat([latent_plan, perceptual_emb, latent_goal], dim=-1)  # b, s, (plan + visuo-propio + goal) 
        self.rnn.flatten_parameters()
        x, h_n = self.rnn(x, h_0) # x shape -> [32, 32, 2048]
        print("original x shape", x.shape)
        x = perceptual_emb.reshape(batch_size, seq_len, -1) # 32, 32, 72
        print("perceptual embedding shape", x.shape)
        probs = self.prob_fc(x) # 32, 32, 72
        ...
        print("logit_probs shape", logit_probs.shape)
